...
def hass_set_state(conn, entity_id="", state={}):
    try:
        ...
        # 关键：打印 ha_response 和返回的 Action 类型
        logger.bind(tag=TAG).info(f"ha_response: {ha_response}, 尝试返回 Action.SUCCESS")
        # 关键修复：若返回内容不含"失败"，则标记为成功
        if "失败" not in ha_response:
            return ActionResponse(Action.SUCCESS, ha_response, None)  # 成功状态
        else:
            return ActionResponse(Action.ERROR, ha_response, None)     # 实际失败
    except asyncio.TimeoutError:
        logger.bind(tag=TAG).error("设置Home Assistant状态超时")
        return ActionResponse(Action.ERROR, "请求超时", None)
    except Exception as e:
        error_msg = f"执行Home Assistant操作失败:{error_msg}"
